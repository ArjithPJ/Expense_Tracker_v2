async function addExpense(e) {
    try{
        ...
        const response = await axios.post('http://44.206.240.170:3000/add-expense/'+token, expenseDetails);
...
async function populateExpenses() {
    ...
    if(buyPremium === "true"){
        ...
        leaderboard.addEventListener("click", async function() {
            try{
                const response = await axios.post("http://44.206.240.170:3000/premium/leaderboard", leaderboardDetails);
                if(response.status ===200){
    ...
    for (let i = 1; i <= lastPage; i++) {
        ...
        button.addEventListener("click", () => {
            axios.get(`http://44.206.240.170:3000/home/?page=${i}&selectedValue=${selectedValue}`,{
                headers: {
